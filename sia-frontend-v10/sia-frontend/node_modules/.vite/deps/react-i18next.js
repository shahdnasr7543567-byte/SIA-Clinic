...
function n(n2) {
  ...
  for (var a2 = new RegExp(e), c2 = null; null !== (c2 = a2.exec(n2)); ) if (c2[0].trim()) if (c2[1]) {
    var l = c2[1].trim(), o = [l, ""];
    l.indexOf("=") > -1 && (o = l.split("=")), r2.attrs[o[0]] = o[1], a2.lastIndex--;
...
function a(t2, e2) {
  switch (e2.type) {
    ...
    case "tag":
      return t2 += "<" + e2.name + (e2.attrs ? function(t3) {
        var e3 = [];
        for (var n2 in t3) e3.push(n2 + '="' + t3[n2] + '"');
        return e3.length ? " " + e3.join(" ") : "";
      }(e2.attrs) : "") + (e2.voidElement ? "/>" : ">"), e2.voidElement ? t2 : t2 + e2.children.reduce(a, "") + "</" + e2.name + ">";
    case "comment":
      return t2 + "<!--" + e2.comment + "-->";
  }
}
var c = { parse: function(t2, e2) {
  e2 || (e2 = {}), e2.components || (e2.components = s);
  var a2, c2 = [], l = [], o = -1, u = false;
  if (0 !== t2.indexOf("<")) {
    var h = t2.indexOf("<");
    c2.push({ type: "text", content: -1 === h ? t2 : t2.substring(0, h) });
  }
  for (var v, f = []; v = r.exec(t2); ) f.push(v);
  return f.forEach(function(t3, e3) {
    var n2 = t3[0];
    if (n2 && !n2.startsWith("<!--")) {
      for (var r2 = 0, i2 = 0, s2 = -1, a3 = null, c3 = 0; c3 < n2.length; c3++) {
        var l2 = n2.charAt(c3);
        a3 ? l2 === a3 && (a3 = null) : '"' === l2 || "'" === l2 ? a3 = l2 : "<" === l2 ? 2 == ++r2 && (s2 = c3) : ">" === l2 && i2++;
      }
      var o2 = s2 > -1 && /[a-zA-Z0-9\-!/]/.test(n2.charAt(s2 + 1));
      if (r2 > i2 && o2) {
        var u2 = n2.substring(0, s2), h2 = n2.substring(u2.length);
        f[e3][0] = h2, f[e3].index += u2.length;
      }
    }
  }), f.forEach(function(r2, s2) {
    var h2 = r2[0];
    if (h2) {
      var v2 = r2.index;
      if (u) {
        if (h2 !== "</" + a2.name + ">") return;
        u = false;
      }
      var m, d, p = "/" !== h2.charAt(1), x = h2.startsWith("<!--"), g = v2 + h2.length, y = t2.charAt(g), E = f[s2 + 1];
      if ("<" === y && E) {
        var A = t2.substring(g, E.index);
        m = A.split("<").length > A.split(">").length;
      }
      if (x) {
        var b = n(h2);
        if (o < 0) return c2.push(b), c2;
        (d = l[o]).children.push(b);
        var O = t2.slice(g, E ? E.index : void 0);
        return O.length > 0 && d.children.push({ type: "text", content: O }), c2;
      }
      if (p && (o++, "tag" === (a2 = n(h2)).type && e2.components[a2.name] && (a2.type = "component", u = true), a2.voidElement || u || !y || "<" === y || a2.children.push({ type: "text", content: t2.slice(g, E ? E.index : void 0) }), 0 === o && c2.push(a2), (d = l[o - 1]) && d.children.push(a2), l[o] = a2), (!p || a2.voidElement) && (o > -1 && (a2.voidElement || a2.name === h2.slice(2, -1)) && (o--, a2 = -1 === o ? c2 : l[o]), !u && ("<" !== y || m) && y)) {
        d = -1 === o ? c2 : l[o].children;
        var W = E ? E.index : -1, j = t2.slice(g, -1 === W ? void 0 : W);
        i.test(j) && (j = " "), (W > -1 && o + d.length >= 0 || " " !== j) && d.push({ type: "text", content: j });
      }
    }
  }), c2;
